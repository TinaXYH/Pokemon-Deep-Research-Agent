...
from ..core.models import (AgentCapability, AgentConfig, AgentType, Task,
                           TaskPriority, TaskStatus)
from ..core.vector_memory import (VectorMemorySystem,
                                  create_vector_memory_system)
...
class PokemonDeepResearchSystem:
    ...
    async def _initialize_agents(self) -> None:
        ...
        self.agent_configs = {
            "coordinator": AgentConfig(
                ...
                capabilities=[
                    AgentCapability(
                        name="query_processing", description="Process user queries"
                    ),
                    AgentCapability(
                        name="workflow_coordination",
                        description="Coordinate agent workflows",
                    ),
                    AgentCapability(
                        name="result_synthesis", description="Synthesize final results"
                    ),
                ],
            ...
            "task_manager": AgentConfig(
                ...
                capabilities=[
                    AgentCapability(
                        ...
                        description="Break down complex queries",
                    ),
                    AgentCapability(
                        name="task_optimization", description="Optimize task execution"
                    ),
                    AgentCapability(
            ...
            "pokeapi_researcher": AgentConfig(
                ...
                capabilities=[
                    AgentCapability(
                        name="pokemon_data_retrieval",
                        description="Retrieve Pokemon data",
                    ),
                    AgentCapability(
                        name="move_data_retrieval", description="Retrieve move data"
                    ),
                    AgentCapability(
                        name="type_data_retrieval", description="Retrieve type data"
                    ),
                    AgentCapability(
                        name="ability_data_retrieval",
                        description="Retrieve ability data",
                    ),
                    AgentCapability(
                        name="location_data_retrieval",
            ...
            "battle_strategist": AgentConfig(
                ...
                capabilities=[
                    AgentCapability(
                        name="team_analysis", description="Analyze team compositions"
                    ),
                    AgentCapability(
                        name="competitive_analysis",
                        description="Competitive viability analysis",
                    ),
                    AgentCapability(
                        name="matchup_analysis", description="Pokemon matchup analysis"
                    ),
                    AgentCapability(
                        name="strategy_development",
                        description="Develop battle strategies",
                    ),
                ],
                max_concurrent_tasks=5,
            ),
            "pokemon_analyst": AgentConfig(
                ...
                capabilities=[
                    ...
                    AgentCapability(
                        name="ability_analysis", description="Ability analysis"
                    ),
                    AgentCapability(name="move_analysis", description="Move analysis"),
                    AgentCapability(
                        name="type_effectiveness_analysis",
            ...
            "report_generator": AgentConfig(
                ...
                capabilities=[
                    AgentCapability(
                        ...
                        description="Synthesize research reports",
                    ),
                    AgentCapability(
    ...
    async def _system_health_check(self) -> Dict[str, Any]:
        ...
        health_status["components"]["message_bus"] = (
            "healthy" if self.message_bus else "unhealthy"
        )
        health_status["components"]["task_channel"] = (
            "healthy" if self.task_channel else "unhealthy"
        )
        health_status["components"]["llm_client"] = (
            "healthy" if self.llm_client else "unhealthy"
        )
        health_status["components"]["pokeapi_client"] = (
            "healthy" if self.pokeapi_client else "unhealthy"
        )
        ...
        unhealthy_agents = [
            k for k, v in health_status["agents"].items() if v["status"] == "unhealthy"
        ]
    ...
    async def _wait_for_query_completion(
        self, query_id: UUID, timeout: int
    ) -> Dict[str, Any]:
        ...
        while (datetime.now() - start_time).total_seconds() < timeout:
            ...
            if task_status == TaskStatus.COMPLETED:
                ...
                return {
                    ...
                    "result": result,
                    "processing_time": (
                        datetime.now() - query_info["start_time"]
                    ).total_seconds(),
                    "timestamp": datetime.now().isoformat(),
                }
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        if not self.is_running:
            return {"status": "stopped", "message": "System is not running"}
        ...
        component_status = {
            "message_bus": {
                ...
                "active_subscriptions": (
                    len(self.message_bus.subscriptions) if self.message_bus else 0
                ),
            },
            "task_channel": {
                "status": "healthy" if self.task_channel else "unhealthy",
                "pending_tasks": (
                    len(self.task_channel.pending_tasks) if self.task_channel else 0
                ),
            },
            "llm_client": {
                "status": "healthy" if self.llm_client else "unhealthy",
                "usage_stats": (
                    self.llm_client.get_usage_stats() if self.llm_client else {}
                ),
            },
            "pokeapi_client": {
                "status": "healthy" if self.pokeapi_client else "unhealthy",
                "cache_stats": (
                    self.pokeapi_client.get_cache_stats() if self.pokeapi_client else {}
                ),
            },
        }
    ...
    async def _print_system_status(self) -> None:
        ...
        for agent_id, agent_status in status["agents"].items():
            ...
            print(
                f"  {running_status} {agent_status['name']}: {agent_status['completed_tasks']} tasks completed"
            )
        print()
    ...
    async def _display_query_result(self, result: Dict[str, Any]) -> None:
        ...
        if status == "completed":
            ...
            if "synthesis" in query_result:
                ...
                print(
                    f"📊 Analysis: {synthesis.get('synthesis_text', 'Analysis completed')}"
                )
                print()
                ...
                print(
                    f"📚 Sources: {', '.join(query_result.get('sources', ['Analysis']))}"
                )
                print()
