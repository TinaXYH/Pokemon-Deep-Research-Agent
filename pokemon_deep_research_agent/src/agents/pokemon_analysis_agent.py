...
class PokemonAnalysisAgent(BaseAgent):
    ...
    def _analyze_pokemon_stats(self, pokemon_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stat_name, value in base_stats.items():
            ...
            if value >= benchmarks.get("excellent", 120):
                strengths.append(
                    {"stat": stat_name, "value": value, "rating": "excellent"}
                )
    ...
    def _analyze_type_synergy(self, types: List[str]) -> Dict[str, Any]:
        ...
        type1_coverage = set(
            self.type_effectiveness.get(type1, {}).get("super_effective", [])
        )
        type2_coverage = set(
            self.type_effectiveness.get(type2, {}).get("super_effective", [])
        )
    ...
    def _analyze_pokemon_abilities(
        ...
        for ability in abilities:
            ...
            ability_info = {
                ...
                "competitive_viability": self._assess_ability_competitive_viability(
                    ability_name
                ),
            }
        ...
        best_ability = max(
            ability_analysis, key=lambda x: self._ability_score(x["impact_rating"])
        )
        ...
        return {
            ...
            "ability_versatility": (
                "high"
                if len(abilities) > 2
                else "medium" if len(abilities) == 2 else "low"
            ),
        }
    ...
    async def _generate_pokemon_assessment(
        ...
        try:
            ...
            return {
                ...
                "training_difficulty": self._assess_training_difficulty(
                    stat_analysis, type_analysis
                ),
    ...
    def _assess_training_difficulty(
        self, stat_analysis: Dict[str, Any], type_analysis: Dict[str, Any]
    ) -> str:
        """Assess training difficulty based on stats and typing."""
        total_stats = stat_analysis.get("total_stats", 0)
        weakness_count = type_analysis.get("defensive_effectiveness", {}).get(
            "weakness_count", 0
        )
    ...
    def _analyze_competitive_viability(
        ...
        return {
            ...
            "competitive_roles": self._suggest_competitive_roles(
                primary_role, stat_analysis
            ),
            "meta_relevance": "requires_current_meta_analysis",
        }

    def _suggest_competitive_roles(
        self, primary_role: str, stat_analysis: Dict[str, Any]
    ) -> List[str]:
    ...
    def _calculate_detailed_stats(self, pokemon_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stat_name, base_value in base_stats.items():
            if stat_name == "hp":
                # HP calculation
                level_50_stats[stat_name] = (
                    int((2 * base_value + 31 + 63) * 50 / 100) + 10
                )
                level_100_stats[stat_name] = (
                    int((2 * base_value + 31 + 252) * 100 / 100) + 10
                )
            else:
                # Other stats calculation
                level_50_stats[stat_name] = int(
                    ((2 * base_value + 31 + 63) * 50 / 100) + 5
                )
                level_100_stats[stat_name] = int(
                    ((2 * base_value + 31 + 252) * 100 / 100) + 5
                )
    ...
    def _calculate_stat_ranges(
        ...
        for stat_name, base_value in base_stats.items():
            if stat_name == "hp":
                ...
                min_stat = (
                    int(((2 * base_value) * 100 / 100) + 5) * 0.9
                )  # With hindering nature
                max_stat = (
                    int(((2 * base_value + 31 + 252) * 100 / 100) + 5) * 1.1
                )  # With beneficial nature

            ranges[stat_name] = {"min": int(min_stat), "max": int(max_stat)}
    ...
    async def _handle_comparative_analysis(self, task: Task) -> Dict[str, Any]:
        ...
        if len(pokemon_list) < 2:
            return {
                "error": "At least 2 Pokemon required for comparison",
                "confidence": 0.0,
            }
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a Pokemon comparison expert.",
                    },
