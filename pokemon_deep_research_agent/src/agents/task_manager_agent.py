...
from ..core.models import AgentConfig, Message, MessageType, Task, TaskPriority
...
class TaskManagerAgent(BaseAgent):
    ...
    async def _handle_query_decomposition(self, task: Task) -> Dict[str, Any]:
        ...
        decompose_func = self.decomposition_templates.get(
            query_type, self._decompose_general_query
        )
            ...
            tasks.append(
                {
                    ...
                    "parameters": {
                        ...
                        "tier_filter": (
                            "OU,UU,RU" if difficulty == "competitive" else "all"
                        ),
                    },
                }
            )
            ...
            tasks.append(
                {
                    ...
                    "priority": 2,
                    "complexity": "low",
                    "parameters": {"focus": "training_ease", "beginner_friendly": True},
    ...
    async def _decompose_comparison_query(
        ...
        comparison_criteria = parameters.get(
            "comparison_criteria", ["stats", "abilities"]
        )
            ...
            tasks.append(
                {
                    ...
                    "priority": 3,
                    "complexity": "medium",
                    "parameters": {"pokemon_name": pokemon, "detailed_analysis": True},
                }
            )
        ...
        tasks.append(
            {
                ...
                "dependencies": (
                    list(range(len(pokemon_mentioned))) if pokemon_mentioned else []
                ),
            }
        )
        ...
        try:
            ...
            response = await self.llm_client.chat_completion(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a Pokemon research task planner.",
                    },
    ...
    async def _optimize_task_execution(
        ...
        dependent_tasks.sort(
            key=lambda t: (min(t["dependencies"]), -t.get("priority", 2))
        )
    ...
    def _estimate_task_duration(self, task: Dict[str, Any]) -> int:
        ...
        base_duration = {"low": 15, "medium": 45, "high": 120}
    ...
    def _suggest_optimizations(
        self, tasks: List[Dict[str, Any]], bottlenecks: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    async def handle_message(self, message: Message) -> None:
        ...
        self.logger.debug(
            f"Task manager received message: {message.message_type} from {message.sender}"
        )
        ...
        if message.message_type == MessageType.TASK_ASSIGNMENT:
            ...
            if content.get("action") == "decompose_query":
                ...
                try:
                    ...
                    self.logger.info(
                        f"Decomposition completed, sending response to {message.sender}"
                    )
                    ...
                    await self.message_bus.publish(
                        f"agent.{message.sender}", response_message
                    )
                    ...
                    await self.message_bus.publish(
                        f"agent.{message.sender}", error_response
                    )
