...
from ..core.models import (
    AgentConfig, AgentType, Message, MessageType, Task, TaskStatus
)
...
class BaseAgent(ABC):
    ...
    async def start(self) -> None:
        ...
        await self.message_bus.subscribe(f"agent.{self.config.agent_id}", self._handle_message)
    ...
    async def stop(self) -> None:
        ...
        await self.message_bus.unsubscribe(f"agent.{self.config.agent_id}", self._handle_message)
        ...
        while self.is_running:
            try:
                ...
                task = await self.task_channel.get_next_task(
                    ...
                    [cap.name for cap in self.config.capabilities]
                )
    ...
    async def _process_task_wrapper(self, task: Task) -> None:
        ...
        try:
            ...
            await self.task_channel.fail_task(task.id, error_message, self.config.agent_id)
    ...
    async def _wait_for_current_tasks(self) -> None:
        ...
        if self.current_tasks:
            self.logger.warning(f"Stopping with {len(self.current_tasks)} tasks still running")
