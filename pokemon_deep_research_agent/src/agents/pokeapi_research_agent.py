...
class PokéAPIResearchAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.competitive_pokemon = [
            "garchomp", "metagross", "salamence", "tyranitar", "dragonite",
            "charizard", "blastoise", "venusaur", "alakazam", "gengar",
            "machamp", "golem", "lapras", "snorlax", "umbreon", "espeon"
        ]
        
        self.beginner_friendly_pokemon = [
            "pikachu", "eevee", "lucario", "gardevoir", "blaziken",
            "swampert", "sceptile", "infernape", "empoleon", "torterra"
        ]
    ...
    async def _handle_pokemon_research(self, task: Task) -> Dict[str, Any]:
        ...
        if pokemon_name:
            ...
            limited_pokemon = type_pokemon[:20] if len(type_pokemon) > 20 else type_pokemon
            
            for pokemon_ref in limited_pokemon:
                try:
                    pokemon_data = await self._get_detailed_pokemon_data(
                        pokemon_ref["pokemon"]["name"],
                        include_stats, include_moves, include_abilities
                    )
                    results.append(pokemon_data)
                except Exception as e:
                    self.logger.warning(f"Failed to get data for {pokemon_ref['pokemon']['name']}: {e}")
                    continue
    ...
    async def _handle_pokemon_identification(self, task: Task) -> Dict[str, Any]:
        ...
        if "easy" in query_lower and "train" in query_lower:
        ...
        elif "ruby" in query_lower or "sapphire" in query_lower or "emerald" in query_lower:
            # Hoenn region Pokemon that are beginner-friendly
            hoenn_beginner = ["blaziken", "swampert", "sceptile", "gardevoir", "metagross"]
            ...
            identified_pokemon = ["pikachu", "charizard", "blastoise", "venusaur", "alakazam"]
        
        # Get detailed data for identified Pokemon
        results = []
        for pokemon_name in identified_pokemon:
            try:
                pokemon_data = await self._get_detailed_pokemon_data(pokemon_name, True, True, True)
                results.append(pokemon_data)
            except Exception as e:
                self.logger.warning(f"Failed to get data for {pokemon_name}: {e}")
                continue
        ...
        return {
            ...
            "identification_reasoning": self._explain_identification(query_context, identified_pokemon),
            "confidence": 0.8,
            "sources": ["pokeapi.co"]
        }
    ...
    def _explain_identification(self, query: str, pokemon_list: List[str]) -> str:
        ...
        if "easy" in query_lower and "train" in query_lower:
            return "Identified beginner-friendly Pokemon with good stats and easy movesets"
    ...
    async def _handle_battle_research(self, task: Task) -> Dict[str, Any]:
        ...
        battle_analysis = await self._analyze_battle_viability(pokemon_data, battle_format)
        ...
        # Recommend movesets
        moveset_recommendations = await self._recommend_movesets(pokemon_data, battle_format)
    ...
    async def _handle_location_research(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            for area_ref in areas:
                try:
                    area_data = await self.pokeapi_client.get_location_area(area_ref["name"])
                    ...
                    self.logger.warning(f"Failed to get area data for {area_ref['name']}: {e}")
                    continue
    ...
    async def _handle_location_search(self, task: Task) -> Dict[str, Any]:
        ...
        if "sea" in criteria_lower or "water" in criteria_lower:
            suggested_locations = [
                {"name": "route-19", "description": "Water route with many Water-type Pokemon"},
                {"name": "seafoam-islands", "description": "Cave system near the sea"},
                {"name": "cinnabar-island", "description": "Island location with water access"}
            ]
        elif "cave" in criteria_lower:
            suggested_locations = [
                {"name": "rock-tunnel", "description": "Dark cave with Rock and Ground types"},
                {"name": "cerulean-cave", "description": "Deep cave with powerful Pokemon"}
            ]
        elif "forest" in criteria_lower or "grass" in criteria_lower:
            suggested_locations = [
                {"name": "viridian-forest", "description": "Forest with Bug and Grass types"},
    ...
    async def _handle_general_research(self, task: Task) -> Dict[str, Any]:
        ...
        if potential_pokemon:
            ...
            return {
                ...
                "suggestions": ["Try mentioning specific Pokemon names", "Use type-based queries", "Specify game versions"],
                "confidence": 0.3,
                "sources": ["pokeapi.co"]
            }
    ...
    def _analyze_stats(self, stats: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if stat_dict.get("attack", 0) > 100 or stat_dict.get("special-attack", 0) > 100:
            role = "attacker"
        elif stat_dict.get("defense", 0) > 100 or stat_dict.get("special-defense", 0) > 100:
            role = "tank"
    ...
    def _analyze_moves(self, moves: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        level_moves = [m for m in moves if any(d["move_learn_method"]["name"] == "level-up" for d in m["version_group_details"])]
        tm_moves = [m for m in moves if any(d["move_learn_method"]["name"] == "machine" for d in m["version_group_details"])]
        
        return {
            "total_moves": total_moves,
            "level_up_moves": len(level_moves),
            "tm_moves": len(tm_moves),
            "move_variety": "high" if total_moves > 50 else "medium" if total_moves > 25 else "low"
        }
    ...
    def _analyze_abilities(self, abilities: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        hidden_abilities = [a["ability"]["name"] for a in abilities if a.get("is_hidden", False)]
    ...
    def _analyze_type_effectiveness(self, types: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "defensive_rating": self._calculate_defensive_rating(effective_weaknesses, effective_resistances, immunities)
        }
    
    def _calculate_defensive_rating(self, weaknesses: set, resistances: set, immunities: set) -> str:
    ...
    async def _analyze_battle_viability(self, pokemon_data: Dict[str, Any], battle_format: str) -> Dict[str, Any]:
    ...
    async def _recommend_movesets(self, pokemon_data: Dict[str, Any], battle_format: str) -> List[Dict[str, Any]]:
        """Recommend movesets for the Pokemon."""
        # This is a simplified implementation
        # In practice, you'd analyze the Pokemon's stats, type, and available moves
        
        suggested_role = pokemon_data.get("stat_analysis", {}).get("suggested_role", "balanced")
    ...
    def _identify_counters(self, types: List[str], pokemon_data: Dict[str, Any]) -> List[Dict[str, Any]]:
