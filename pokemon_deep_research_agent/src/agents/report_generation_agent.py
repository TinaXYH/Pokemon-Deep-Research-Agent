...
from ..core.models import (
    AgentConfig, Message, MessageType, Task, ResearchResult
)
...
class ReportGenerationAgent(BaseAgent):
    ...
    async def _handle_report_synthesis(self, task: Task) -> Dict[str, Any]:
        ...
        template = self.report_templates.get(report_type, self.report_templates["pokemon_analysis"])
        
        # Synthesize findings
        synthesis = await self._synthesize_research_findings(research_findings, query_context)
        ...
        markdown_report = await self._create_markdown_report(structured_report, template)
        
        # Generate executive summary
        executive_summary = await self._generate_executive_summary(synthesis, query_context)
    ...
    async def _synthesize_research_findings(
        ...
        synthesis_text = await self._generate_synthesis_text(findings, context, key_insights)
    ...
    async def _extract_key_insights(
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {"role": "system", "content": "You are a Pokemon research analyst expert at extracting key insights."},
    ...
    def _create_findings_summary(self, findings: List[Dict[str, Any]]) -> str:
        ...
        for i, finding in enumerate(findings[:10]):  # Limit to first 10 findings
            ...
            summary_parts.append(f"Finding {i+1}: {finding_type} (confidence: {confidence})")
    ...
    def _identify_patterns(self, findings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        patterns.append({
            ...
            "significance": "high" if avg_confidence >= 0.8 else "medium" if avg_confidence >= 0.6 else "low"
        })
    ...
    async def _generate_synthesis_text(
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {"role": "system", "content": "You are a Pokemon research expert creating comprehensive analysis reports."},
    ...
    def _assess_data_quality(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        quality_score = (avg_confidence * 0.6 + 
    ...
    def _calculate_completeness_score(
        ...
        expected_types = {
            "team_building": ["pokemon_research", "competitive_analysis", "team_analysis"],
            "individual_analysis": ["pokemon_research", "stat_analysis", "type_analysis"],
    ...
    async def _generate_executive_summary_section(
        ...
        return {
            ...
            "confidence_level": synthesis.get("data_quality", {}).get("assessment", "unknown"),
            "completeness": synthesis.get("completeness_score", 0.0),
    ...
    async def _generate_pokemon_overview_section(
        ...
        pokemon_findings = synthesis.get("categorized_findings", {}).get("pokemon_data", [])
        
        return {
            "content_type": "overview",
            "pokemon_analyzed": len(pokemon_findings),
            "data_sources": synthesis.get("data_quality", {}).get("source_diversity", 0),
    ...
    async def _generate_statistical_analysis_section(
        ...
        stat_findings = synthesis.get("categorized_findings", {}).get("statistical_analysis", [])
    ...
    def _extract_key_statistics(self, stat_findings: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _generate_competitive_viability_section(
        ...
        competitive_findings = synthesis.get("categorized_findings", {}).get("competitive_analysis", [])
    ...
    async def _generate_conclusion_section(
        ...
        return {
            ...
            "conclusion_text": synthesis.get("synthesis_text", "")[-300:],  # Last 300 chars
            "data_quality": synthesis.get("data_quality", {}).get("assessment", "unknown"),
            "completeness": synthesis.get("completeness_score", 0.0)
        }
    ...
    async def _generate_team_overview_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "team_overview", "overview": "Team building analysis overview"}
    
    async def _generate_role_distribution_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "role_distribution", "distribution": "Team role analysis"}
    
    async def _generate_type_coverage_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "type_coverage", "coverage": "Type coverage analysis"}
    
    async def _generate_synergy_assessment_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "synergy", "assessment": "Team synergy analysis"}
    
    async def _generate_threat_analysis_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "threats", "analysis": "Threat analysis"}
    
    async def _generate_alternatives_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "alternatives", "options": "Alternative options"}
    
    async def _generate_final_recommendations_section(self, synthesis: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        return {"content_type": "final_recommendations", "recommendations": "Final recommendations"}
    ...
    async def _create_markdown_report(
        ...
        markdown_parts.append(f"**Report Type:** {structured_report.get('template_type', 'General')}")
        markdown_parts.append("")
        ...
        for section_name in template["sections"]:
            if section_name in sections:
                ...
                formatted_content = self._format_section_content(section_data, content_type)
    ...
    def _format_section_content(self, section_data: Dict[str, Any], content_type: str) -> str:
        ...
        if content_type == "summary":
            ...
            for i, rec in enumerate(recommendations, 1):
                content.append(f"### {i}. {rec.get('recommendation', 'Recommendation')}")
                content.append(f"**Priority:** {rec.get('priority', 'Medium')}")
                content.append(f"**Rationale:** {rec.get('rationale', 'Analysis-based recommendation')}")
                content.append("")
            ...
            # Generic formatting
            text_fields = ["content_text", "overview", "analysis", "assessment", "description"]
    ...
    async def _generate_executive_summary(
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {"role": "system", "content": "You are a Pokemon research analyst creating executive summaries."},
            ...
            return {
                ...
                "key_insight": key_insights[0]["insight"] if key_insights else "Research completed",
    ...
    async def _handle_data_visualization(self, task: Task) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_formats": ["markdown_tables", "stat_charts", "type_effectiveness_charts"],
            "confidence": 0.8
        }
    ...
    def _format_recommendation_list(self, recommendations: List[Dict[str, Any]]) -> str:
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            list_lines.append(f"{i}. {priority_symbol} {rec.get('recommendation', 'Recommendation')}")
