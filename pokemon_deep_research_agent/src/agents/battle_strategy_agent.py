...
class BattleStrategyAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.battle_formats = {
            "singles": {
                ...
                "key_roles": ["physical_sweeper", "special_sweeper", "wall", "support", "lead", "revenge_killer"]
            },
            "doubles": {
                "team_size": 6,
                "active_pokemon": 2,
                "common_strategies": ["trick_room", "tailwind", "weather", "protect_spam"],
                "key_roles": ["damage_dealer", "support", "trick_room_setter", "speed_control", "redirection"]
            },
            "vgc": {
                "team_size": 6,
                "active_pokemon": 2,
                "common_strategies": ["restricted_legendary", "weather_control", "speed_control"],
                "key_roles": ["restricted", "support", "damage", "speed_control", "utility"]
            }
        }
        
        # Common competitive items and their strategic uses
        self.competitive_items = {
            "choice_band": {"type": "offensive", "effect": "+50% Attack, locks move", "best_for": ["physical_sweeper"]},
            "choice_specs": {"type": "offensive", "effect": "+50% Sp. Attack, locks move", "best_for": ["special_sweeper"]},
            "choice_scarf": {"type": "speed", "effect": "+50% Speed, locks move", "best_for": ["revenge_killer"]},
            "life_orb": {"type": "offensive", "effect": "+30% damage, 10% recoil", "best_for": ["mixed_attacker"]},
            "leftovers": {"type": "defensive", "effect": "1/16 HP recovery per turn", "best_for": ["wall", "support"]},
            "focus_sash": {"type": "utility", "effect": "Survive 1 HP from full HP", "best_for": ["lead", "glass_cannon"]},
            "assault_vest": {"type": "defensive", "effect": "+50% Sp. Defense, no status moves", "best_for": ["special_tank"]},
            "rocky_helmet": {"type": "defensive", "effect": "1/6 damage to contact moves", "best_for": ["physical_wall"]}
        }
        ...
        self.team_archetypes = {
            "hyper_offense": {
                ...
                "roles": ["lead", "physical_sweeper", "special_sweeper", "revenge_killer", "wallbreaker", "cleaner"],
            ...
            "balanced": {
                ...
                "roles": ["physical_wall", "special_wall", "physical_sweeper", "special_sweeper", "support", "pivot"],
            ...
            "stall": {
                ...
                "roles": ["physical_wall", "special_wall", "cleric", "hazard_setter", "phazer", "wincon"],
                "strategy": "Wear down opponents through passive damage and recovery"
            },
            "weather": {
                "description": "Team built around weather conditions",
                "roles": ["weather_setter", "weather_abuser", "weather_abuser", "support", "check", "backup_setter"],
    ...
    async def _handle_team_analysis(self, task: Task) -> Dict[str, Any]:
        ...
        format_config = self.battle_formats.get(battle_format, self.battle_formats["singles"])
        
        # Analyze team requirements
        team_requirements = self._analyze_team_requirements(battle_format, type_constraint, difficulty_level)
    ...
    async def _handle_competitive_analysis(self, task: Task) -> Dict[str, Any]:
        ...
        usage_analysis = self._simulate_usage_stats(battle_format) if usage_stats else {}
    ...
    async def _handle_matchup_analysis(self, task: Task) -> Dict[str, Any]:
        ...
        format_implications = self._analyze_format_implications(pokemon_a, pokemon_b, battle_format)
    ...
    async def _handle_strategy_development(self, task: Task) -> Dict[str, Any]:
        ...
        counters = self._identify_counters_and_threats(pokemon_focus) if include_counters else []
    ...
    async def _handle_team_recommendation(self, task: Task) -> Dict[str, Any]:
        ...
        alternatives = self._generate_alternative_options(team_recommendations, battle_format)
    ...
    def _analyze_team_requirements(
        ...
        requirements = {
            ...
            "difficulty_considerations": self._get_difficulty_considerations(difficulty_level)
        }
    ...
    def _identify_required_roles(self, battle_format: str, type_constraint: Optional[str]) -> List[Dict[str, Any]]:
        ...
        for role in base_roles[:4]:  # Focus on core roles
            role_info = {
                ...
                "type_constraint_compatible": self._check_role_type_compatibility(role, type_constraint)
            }
    ...
    def _check_role_type_compatibility(self, role: str, type_constraint: Optional[str]) -> bool:
    ...
    def _analyze_type_coverage_needs(self, type_constraint: Optional[str]) -> Dict[str, Any]:
        ...
        return type_coverage.get(type_constraint, {
    ...
    async def _generate_team_building_guidelines(
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {"role": "system", "content": "You are a Pokemon competitive team building expert."},
    ...
    def _analyze_tier_viability(self, battle_format: str) -> Dict[str, Any]:
        ...
        return {
            "OU": {"viability": "high", "usage": "25%+", "description": "Overused tier - most viable Pokemon"},
            "UU": {"viability": "medium-high", "usage": "10-25%", "description": "Underused tier - solid alternatives"},
            "RU": {"viability": "medium", "usage": "5-10%", "description": "Rarely used tier - niche options"},
            "NU": {"viability": "low-medium", "usage": "1-5%", "description": "Never used tier - specialized roles"},
            "PU": {"viability": "low", "usage": "<1%", "description": "Partially used tier - very niche"}
        }
    ...
    def _analyze_format_implications(self, pokemon_a: str, pokemon_b: str, battle_format: str) -> Dict[str, Any]:
    ...
    async def _generate_matchup_recommendations(
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {"role": "system", "content": "You are a Pokemon battle strategy expert."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.6
            )
    ...
    async def _develop_core_strategy(self, battle_format: str, pokemon_focus: List[str]) -> Dict[str, Any]:
        ...
        core_strategy = {
            ...
            "win_conditions": self._identify_win_conditions(pokemon_focus, battle_format),
    ...
    def _identify_win_conditions(self, pokemon_focus: List[str], battle_format: str) -> List[str]:
        """Identify primary win conditions."""
        if not pokemon_focus:
            return ["Maintain board control", "Apply consistent pressure", "Capitalize on opponent mistakes"]
    ...
    def _develop_game_plan(self, strategy_type: str, battle_format: str) -> Dict[str, Any]:
    ...
    def _identify_counters_and_threats(self, pokemon_focus: List[str]) -> List[Dict[str, Any]]:
    ...
    async def _generate_strategic_recommendations(
        ...
        try:
            response = await self.llm_client.chat_completion(
                messages=[
                    {"role": "system", "content": "You are a Pokemon competitive strategy expert."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7
            )
    ...
    def _analyze_team_synergy(self, team_recommendations: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _identify_team_weaknesses(self, team_recommendations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        return [
            {
                ...
                "changes": ["Replace slot 1 with weather setter", "Adjust items for weather"]
            },
            {
    ...
    def _analyze_current_meta(self, battle_format: str, tier_focus: str) -> Dict[str, Any]:
        """Analyze current meta state."""
        return {
            "meta_health": "diverse",
    ...
    def _predict_meta_shifts(self, battle_format: str, meta_state: Dict[str, Any]) -> Dict[str, Any]:
