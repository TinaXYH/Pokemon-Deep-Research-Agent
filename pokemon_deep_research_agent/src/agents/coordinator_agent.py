...
from ..core.models import (AgentConfig, AgentType, Message, MessageType,
                           ResearchQuery, Task, TaskPriority, TaskStatus)
...
class CoordinatorAgent(BaseAgent):
    ...
    async def _handle_research_query(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Analyzed query: {original_query} -> Type: {research_query.query_type}"
            )
    ...
    async def _needs_clarification(
        self, query: ResearchQuery, analysis: Dict[str, Any]
    ) -> bool:
        ...
        ambiguity_indicators = [
            ...
            len(analysis.get("pokemon_mentioned", [])) == 0
            and query.query_type == "individual_analysis",
    ...
    async def _request_clarification(
        ...
        try:
            clarification_questions = (
                await self.llm_client.generate_clarification_questions(
    ...
    def _build_clarified_query(
        self, query: ResearchQuery, responses: Dict[str, Any]
    ) -> str:
    ...
    async def _initiate_research_workflow(self, query: ResearchQuery) -> Dict[str, Any]:
        ...
        for i, task_info in enumerate(task_list):
            ...
            self.logger.info(
                f"  Required capability: {task_info.get('required_capability')}"
            )
        ...
        self.logger.info(
            f"Initiated research workflow for query {query.id} with {len(task_ids)} tasks"
        )
    ...
    def _estimate_completion_time(self, task_list: List[Dict[str, Any]]) -> str:
        ...
        complex_tasks = sum(
            1 for task in task_list if task.get("complexity", "medium") == "high"
        )
        medium_tasks = sum(
            1 for task in task_list if task.get("complexity", "medium") == "medium"
        )
    ...
    async def _handle_task_failure(self, message: Message) -> None:
        ...
        self.logger.warning(
            f"Task {task_id} failed for query {query_id}: {error_message}"
        )
    ...
    async def handle_message(self, message: Message) -> None:
        ...
        self.logger.debug(
            f"Coordinator received message: {message.message_type} from {message.sender}"
        )
        ...
        status.update(
            {
                ...
                "total_tasks_tracked": sum(
                    len(tasks) for tasks in self.query_tasks.values()
                ),
            }
        )
        return status
        ...
        try:
            ...
            final_result = {
                ...
                "synthesis": {
                    ...
                    "key_insights": [
                        {
                            "insight": "Research analysis completed",
                            "importance": "medium",
                        }
                    ],
