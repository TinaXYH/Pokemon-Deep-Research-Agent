...
class FunctionalityTester:
    ...
    def setup_clients(self):
        ...
        self.llm_client = LLMClient(
            api_key=self.api_key, model="gpt-4.1-mini", max_tokens=2000, temperature=0.3
        )
    ...
    async def test_specification_workflow(self):
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            try:
                ...
                print(
                    f"   Query Classification: {spec.query_type.value} ({gen_time:.2f}s)"
                )
    ...
    async def test_research_execution(self):
        ...
        try:
            ...
            from src.core.specification_manager import (QueryType,
                                                        TaskSpecification)
            ...
            if pokemon_data:
                print(
                    f"   Pokemon Data Retrieved: {pokemon_data.name} ({data_time:.2f}s)"
                )
                print(f"   Types: {[t['type']['name'] for t in pokemon_data.types]}")
                print(
    ...
    async def test_interaction_mode_simulation(self):
        ...
        try:
            ...
            result1 = (
                "Garchomp is a top-tier Dragon/Ground type Pokemon in OU format..."
            )
            ...
            spec2 = await self.spec_manager.analyze_query_and_generate_spec(
                query2, context
            )
            ...
            # Simulate research result
            result2 = (
                "Comparing Garchomp to Dragonite, both are powerful Dragon types..."
            )
