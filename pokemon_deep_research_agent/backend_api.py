    ...
    async def process_query(self, query, conversation_id="default"):
        ...
        try:
            ...
            history = self.conversation_memory.get_conversation_history(
                conversation_id, max_turns=3
            )

            # If conversation doesn't exist, start a new one with the given ID
            if (
                not history
                and conversation_id not in self.conversation_memory.active_conversations
            ):
                ...
                if conversation_id == "default":
                    ...
                    self.conversation_memory.active_conversations["default"] = (
                        context_obj
                    )
            ...
            context_for_classification = {
                "previous_turns": [
                    {"user": turn.user_input, "agent": turn.agent_response}
                    for turn in history
                ],
            ...
            spec = await self.spec_manager.analyze_query_and_generate_spec(
                query, context_for_classification
            )
            ...
            yield {
                ...
                "data": {
                    ...
                    "pokemon_names": (
                        spec.fields.get("target_pokemon", [])
                        if isinstance(spec.fields.get("target_pokemon"), list)
                        else [spec.fields.get("target_pokemon", "")]
                    ),
            ...
            analysis_prompt = self._build_analysis_prompt(
                spec, pokemon_data, context_for_classification
            )
            ...
            report_prompt = self._build_report_prompt(
                spec, pokemon_data, analysis, context_for_classification
            )
        ...
        def generate():
            ...
            try:
                ...
                async def run_research():
                    async for result in research_agent.process_query(
